var Geo=(function() {
	...
	function linesCrossingPoint2D(P1,V1_,P2,V2_,isRay=false,middlePointFallback=false) { //exceptions: true - points are on the same line (rays overlaps) - an infinite number of crosspoints, null - lines are parallel and not in the same line (rays look in same direction) - the crosspoint somewhere in the infinity, false - rays do not intersect or vectors are invalid
		...
		var	c1=-P1[0]*-V1[1]-P1[1]*V1[0]
		var	c2=-P2[1]*V2[0]-P2[0]*-V2[1]
		var dem=-V1[1]*V2[0]+V2[1]*V1[0]
		var x=V1[0]*c2-V2[0]*c1
		var y=-V2[1]*c1+V1[1]*c2
		if (Math.abs(dem)>tol) {
			var R=[x/dem,y/dem]
	...
	function loxDistanceToCrossPoint(R,P){
		...
		if (i>tol || Math.abs(P.location[0])-90<tol) {
	...
	return {
		...
		SphericalTwoLoxodromesIntersectionPoint: function (p1,p2,withDistance=false) {
			...
			if (P===null) {
				if (Math.abs(Math.cos(R1.azimuthRad))<tol)
					return false
