	...
	return {
		...
		SphericalTwoLoxodromesIntersectionPoint: function (p1,p2,withDistance=false) {
			...
			if (interval(R1.point.radius,R2.point.radius)<tol) // same points catch
				...
				if (P===true) //true - loxodromes launched from opposite poles towards the same longitide,
					P= new Point([0,R1.point.mercatorLocation[1]])
				else if (P===false) // false - loxodromes launched from opposite poles towards different longitide, or there is no intersection at all  
			...
			if (!(P instanceof Point)) {// normal
				if (isNaN(R1.point.mercatorLocation[0]) && Math.abs((R1.point.mercatorLocation[1]-P[1])%360)<tol && Math.abs((R2.point.mercatorLocation[1]-P[1])%360)<tol && Math.abs(interval(R1.mercatorVector,R2.mercatorVector)-2)<tol)
					P=new Point([(R1.point.location[0]+R2.point.location[0])/2,R2.point.location[1]]).mercator()
				else if (isNaN(R2.point.mercatorLocation[0]) && Math.abs((R2.point.mercatorLocation[1]-P[1])%360)<tol && Math.abs((R1.point.mercatorLocation[1]-P[1])%360)<tol && Math.abs(interval(R1.mercatorVector,R2.mercatorVector)-2)<tol)
					P=new Point([(R2.point.location[0]+R1.point.location[0])/2,R1.point.location[1]]).mercator()
				else
					P=Point.fromMercator(P)
			} else				
				P.mercator()
			
			if (Math.abs(Math.abs(P.location[0])-90)<tol) // cross point is a pole, so no sense of longitude path as it makes infinite number of loop reaching the pole.
